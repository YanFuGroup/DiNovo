...
PM: Precursor Mass
SM: Sequence Mass
MMT: Max Mass Tolerance(ppm)

to make: abs(PM-SM)/SM * 1e6 <= MMT
and we know PM(from spectra) and MMT(setting by user), search the range of SM

    abs(PM-SM)/SM * 1e6 <=  MMT
 ->        abs(PM-SM)   <=  MMT * SM / 1e6

 ->     -MMT*SM/1e6  <=  SM-PM <=      MMT*SM/1e6

 -> PM - MMT*SM/1e6  <=   SM   <= PM + MMT*SM/1e6 

 -> PM - MMT*SM/1e6  <=   SM  ->  PM <= SM + MMT*SM/1e6  ->  PM <= SM * (1 + MMT/1e6)  ->  SM >= PM/(1 + MMT/1e6)
 && PM + MMT*SM/1e6  >=   SM  ->  PM >= SM - MMT*SM/1e6  ->  PM >= SM * (1 - MMT/1e6)  ->  SM <= PM/(1 - MMT/1e6)
 
 -> PM/(1 + MMT/1e6) <= SM <= PM/(1 - MMT/1e6)
...
def judge_mass_tol_ppm(mass, calc_mass, mass_tol):
    ...
    flag_tol = True if abs(delta)/calc_mass * 1e6 <= mass_tol else False
