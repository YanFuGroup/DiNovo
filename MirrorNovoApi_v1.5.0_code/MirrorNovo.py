...
from data_loader import MirrorNovoDenovoDataset
...
def mirrornovo(args, denovo_input_feature_file,denovo_output_file,process_id):
    ...
    data_reader = MirrorNovoDenovoDataset(args, args.denovo_input_spectrum_file, args.denovo_input_mirror_spectrum_file, denovo_input_feature_file)
    time_rec = time.perf_counter()
    denovo_worker = IonCNNDenovo(args=args,device=device)
    forward_mirrornovo, backward_mirrornovo = build_model(args=args, training=False,device=device)
    model_wrapper = InferenceModelWrapper(forward_mirrornovo, backward_mirrornovo)
    ...
    # print("##### denovo only time:", time_rec)
    # denovo_worker.report_calc_time()
    return None
def rerank(param_name):
    ...
    time1 = time.time()
    data_reader = MirrorNovoDenovoDataset(args, args.denovo_input_spectrum_file, args.denovo_input_mirror_spectrum_file, args.denovo_input_feature_file)
    denovo_worker = IonCNNDenovo(args=args)
    forward_mirrornovo, backward_mirrornovo = build_model(args=args, training=False)
    model_wrapper = InferenceModelWrapper(forward_mirrornovo, backward_mirrornovo)
...
if __name__ == '__main__':
    ...
    args = init_args(param_path)
    create_file(args)
    # print("max_mass_tolerance =", args.max_mass_tolerance)
    # print("mass_tolerance_ppm =", args.mass_tolerance_ppm)
    # exit()
