...
from data_loader import MirrorNovoDenovoDataset, chunks
from writer import DenovoWriter
from member import BeamSearchStartPoint, SearchPath, SearchEntry, DenovoResult, BeamSearchResult, BeamSearchedSequence
import config
...
def get_ion_index(peptide_mass, prefix_mass, input_id_list, direction, try_lys, args):
    """
    ...
    if try_lys == 0:
        b_ions = [candidate_b_mass,
                  ...
                  candidate_b_plus2_charge1 * 0,
                  ]
        y_ions = [candidate_y_mass,
                  candidate_y_mass + config.mass_double_k,
                  candidate_y_H2O,
                  candidate_y_H2O + config.mass_double_k,
                  candidate_y_NH3,
                  candidate_y_NH3 + config.mass_double_k,
                  candidate_y_plus2_charge1,
                  candidate_y_plus2_charge1 + config.mass_double_k / 2,
                  ]
        ...
        b_ions = [candidate_b_mass,
                  candidate_b_mass + config.mass_double_k,
                  candidate_b_H2O,
                  candidate_b_H2O + config.mass_double_k,
                  candidate_b_NH3,
                  candidate_b_NH3 + config.mass_double_k,
                  candidate_b_plus2_charge1,
                  candidate_b_plus2_charge1 + config.mass_double_k / 2,
                  ]
        y_ions = [candidate_y_mass,
                  candidate_y_mass * 0,
                  candidate_y_H2O,
    ...
    if len(input_id_list) == 1:
        internal_by = [internal_ions] + [[0.0] * len(mass_ID_np)] + [[0.0] * len(mass_ID_np)] + [
            [0.0] * len(mass_ID_np)] + [[0.0] * len(mass_ID_np)]
    elif len(input_id_list) == 2:
        internal_2ions = internal_aa_sum + mass_ID_np
        internal_by = [internal_ions, internal_2ions] + [[0.0] * len(mass_ID_np)] + [[0.0] * len(mass_ID_np)] + [
            [0.0] * len(mass_ID_np)]
        ...
        internal_by = [internal_ions, internal_2ions, internal_3ions] + [[0.0] * len(mass_ID_np)] + [
            [0.0] * len(mass_ID_np)]
...
class KnapsackSearcher(object):
    def __init__(self, args, MZ_MAX, knapsack_file):
    ...
    def search_knapsack(self, residual_mass, precursor_mass, peak_mass_tolerance):

        if self.args.mass_tolerance_ppm:
            ...
            mass_upperbound = int(round((residual_mass + (peak_mass_tolerance * precursor_mass / (
                    10 ** 6 - peak_mass_tolerance))) * self.knapsack_aa_resolution))
            ...
            mass_lowerbound = int(round((residual_mass - (peak_mass_tolerance * precursor_mass / (
                    10 ** 6 + peak_mass_tolerance))) * self.knapsack_aa_resolution))
        else:
            mass_upperbound = int(round((residual_mass + peak_mass_tolerance) * self.knapsack_aa_resolution))
            mass_lowerbound = int(round((residual_mass - peak_mass_tolerance) * self.knapsack_aa_resolution))
...
class IonCNNDenovo(object):
    def __init__(self, args, device):
        self.args = args
        self.mass_tol = self.args.max_mass_tolerance
        self.mass_ppm = self.args.mass_tolerance_ppm
        self.MZ_MAX = args.MZ_MAX  # legacy, not used here
        self.beam_size = args.beam_size
        self.knapsack_searcher = KnapsackSearcher(self.args, self.MZ_MAX, args.knapsack)
        self.device = device
        if args.type:
            mass_ID_np[4] = 156.10111
            config.mass_ID[4] = 156.10111
            config.mass_ID_np[4] = 156.10111
            config.mass_AA['R'] = 156.10111
        self.judge_mass_tol = config.judge_mass_tol_ppm if self.mass_ppm else config.judge_mass_tol_da
        ...
        for feature_index in range(num_features):
            ...
            if direction == Direction.forward:
                ...
                path = SearchPath(
                    ...
                    aa_seq_mass=get_start_mass(start_point_batch[feature_index]) + config.try_last_aa_mass_dict[
                        category],
                    score_list=[0.0, 0.0],
                    score_sum=0.0,
                    direction=direction,
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for path in current_path_list:
                    ...
                    if aa_id == last_label:
                        flag = self.judge_mass_tol(precursor_mass, aa_seq_mass, self.mass_tol)
                        if flag:
                        ...
                        continue
                    ion_location = get_ion_index(precursor_mass, aa_seq_mass, aa_id_list, direction_cint_map[direction],
                                                 0, args=self.args)  # [26,8]
                    if direction == Direction.forward:
                    ...
                    mirror_ion_location = get_ion_index(lys_precursor_mass, mirror_aa_seq_mass, mirror_aa_id_list,
                                                        direction_cint_map[direction], 1, args=self.args)
                    ...
                    '''
                    # 这是以ppm为单位计算误差
                    knapsack_candidates = self.knapsack_searcher.search_knapsack(residual_mass, precursor_mass,
                                                                                     self.mass_tol)
            ...
            block_mirror_ion_location = torch.from_numpy(np.array(block_mirror_ion_location)).to(
                self.device)  # [batch, 26, 8, 10]
    ...
    # @staticmethod
    def _get_start_point(self, feature_dp_batch: list) -> tuple:
        ...
        forward_start_point_lists = [BeamSearchStartPoint(prefix_mass=mass_GO,
                                                          ...
                                                          mass_tolerance=self.mass_tol,
                                                          mass_tol_ppm_unit=self.args.mass_tolerance_ppm,
    ...
    # @staticmethod
    def _select_path(self, feature_dp_batch: list, top_candidate_batch: list) -> tuple:
        """
        for each feature, select the best denovo sequence given by MirrorNovo model
        :param feature_dp_batch: list of DenovoData
        ...
        for feature_index in range(feature_batch_size):
            ...
            for beam_search_sequence in candidate_list:
                ...
                sequence_mass += config.mass_ID[config.GO_ID] + config.mass_ID[
                    config.EOS_ID]
                flag = self.judge_mass_tol(precursor_mass, sequence_mass, self.mass_tol)
                if flag:
    ...
    def search_denovo(self, model_wrapper: InferenceModelWrapper,
                      beam_search_reader: MirrorNovoDenovoDataset, denovo_writer: DenovoWriter, process_id):
        ...
        for index, feature_batch_index in enumerate(test_set_iter):
            ...
            print("\rRead process {} {}th/{} batches, time remaining {}h".format(process_id, index, total_batch_num,
                                                                                 (total_batch_num - index) * (
