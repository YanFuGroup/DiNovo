...
def mirrornovo(args, denovo_input_feature_file, denovo_output_file, process_id):
    ...
    if args.type == 1:
        # print(args.type)
        print("lysC & lysN mode")
        args.knapsack = args.knapsack.replace("neucodeknapsack_C_M_IL.npy", "neucodeknapsack_R.npy")
    device = torch.device(f"cuda:{process_id}" if torch.cuda.is_available() and process_id != "-1" else "cpu")
    data_reader = MirrorNovoDenovoDataset(args, args.denovo_input_spectrum_file, args.denovo_input_mirror_spectrum_file,
...
if __name__ == '__main__':
    ...
    # print(args.denovo_input_feature_file)
    # print(args.denovo_output_file)
