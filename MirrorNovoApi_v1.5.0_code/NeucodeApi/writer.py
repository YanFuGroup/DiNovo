...
class DenovoWriter(object):
    def __init__(self, args, denovo_output_file):
        self.args = args
        # print("==="*9)
        # print("self.args.type", self.args.type)
    ...
    def write_beamsearch(self, dda_original_feature: DDAFeature, beam_search_batch: list):
        ...
        for id, searched_sequence in enumerate(new_beam_search_batch):
            if searched_sequence.sequence:
                # print([config.vocab_reverse[_aa] for _aa in searched_sequence.sequence])
                # print("feature_category", feature_category)
                # print("feature_category[-1]", feature_category[-1])
                if feature_category[-1] == 'X':
                    predicted_sequence = ''
                    modification = ''
                else:
                    predicted_sequence = feature_category[-1]
                    # print("predicted_sequence", predicted_sequence)
                    # print("judge 1", predicted_sequence == "R")
                    # print("judge 2", self.args.type+1)
                    # print("judge 2", self.args.type==0)
                    if predicted_sequence == "K":
                        modification = "1,NeuCodeK602[K];"
                    elif predicted_sequence == "R" and self.args.type==0:
                        modification = "1,NeuCodeR004[R];"
                    else:
                        modification = ""
                    
                for aa_id in searched_sequence.sequence:
                    if aa_id == 7:
                        aa, site = 'C', len(predicted_sequence)
                    elif aa_id == 4:
                        aa, site = 'R', len(predicted_sequence)
                    elif aa_id == 13:
                        aa, site = 'K', len(predicted_sequence)
                    elif aa_id == 15:
                        aa, site = 'M', len(predicted_sequence)
                    else:
                        aa, site = config.vocab_reverse[aa_id], ''
                    predicted_sequence += aa
                    if site != '':  # ["8,Oxidation[M];9,QWER[K];", ...]
                        if aa == 'M':
                            modification += str(site + 1) + "," + "Oxidation[M];"
                        if aa == 'C':
                            modification += str(site + 1) + "," + "Carbamidomethyl[C];"
                        if aa == 'K':
                            modification += str(site + 1) + "," + "NeuCodeK602[K];"
                        if aa == 'R' and self.args.type==0:
                            modification += str(site + 1) + "," + "NeuCodeR004[R];"
        ...
        merged_list = list(zip(RANK, SEQUENCE, MODIFICATION, PEPSCORE, AASCORE))
        # 将每一行的元素用\t连接起来
