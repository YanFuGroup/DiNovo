...
class KnapsackSearcher(object):
    ...
    # 我检查过了，这个函数虽然执行次数多，但是花费的计算时间是比较少的；
    # 嘶，本以为能加速，结果并不能q(T(00)T)p
    # 20260101  这个地方要修改为用户设置的质量误差
    def search_knapsack(self, residual_mass,precursor_mass, peak_mass_tolerance):
        ...
        # time_rec = time.perf_counter() - time_rec
        ...
        candidate_aa_id = np.flatnonzero(np.any(self.knapsack_matrix[:, mass_lowerbound_col:(mass_upperbound_col + 1)],
                                                axis=1))
        return candidate_aa_id.tolist()# , time_rec
...
class IonCNNDenovo(object):
    ...
    def report_calc_time(self):
        print("\n\n\n\n")
        print("Calc tolerance time:", self.calc_counter)
        print("Time record:", self.time_record)
        print("\n\n\n\n")
    ...
    def _beam_search(self, model_wrapper: InferenceModelWrapper,
        ...
        :param model_wrapper:
        :param feature_dp_batch: list of DenovoData (DDAFeature)
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for path in current_path_list:
                    ...
                    knapsack_candidates = self.knapsack_searcher.search_knapsack(residual_mass, precursor_mass,self.mass_tol)
    ...
    def _get_start_point(self, feature_dp_batch: list) -> tuple:
        ...
        backward_start_point_lists = [BeamSearchStartPoint(prefix_mass=feature_dp.original_dda_feature.mass - mass_EOS,
                                                           ...
                                                           # mass_tolerance=config.PRECURSOR_MASS_PRECISION_TOLERANCE,
                                                           mass_tolerance=self.mass_tol,
                                                           mass_tol_ppm_unit=self.mass_tol,
    ...
    def _select_path(self, feature_dp_batch: list, top_candidate_batch: list) -> tuple:
        ...
        for feature_index in range(feature_batch_size):
            ...
            for beam_search_sequence in candidate_list:
                ...
                # flag = abs((precursor_mass - sequence_mass) / sequence_mass * 10 ** 6) <= config.PRECURSOR_MASS_PRECISION_TOLERANCE_PPM
                self.time_record += time.perf_counter() - time_rec
                if flag:#ppm
    ...
    def _search_denovo_batch(self, feature_dp_batch: list, model_wrapper: InferenceModelWrapper) -> tuple:
        ...
        feature_batch_size = len(feature_dp_batch)
        start_points_tuple = self._get_start_point(feature_dp_batch)  # return 2 list of BeamSearchStartPoint:class
