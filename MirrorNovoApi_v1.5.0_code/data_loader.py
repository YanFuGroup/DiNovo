...
class MirrorNovoDenovoDataset(Dataset):
        ...
        if os.path.exists(spectrum_location_file):
            ...
            with open(spectrum_path, 'r') as f:
                ...
                while line:
                    ...
                    if ("B" == line[0]) and ("BEGIN IONS" in line):
                        spectrum_location = f.tell()
                    elif ("T" == line[0]) and ("TITLE=" in line):
                        # title = re.split('[=\r\n]', line)[1]
                        title = line[6:-1]  # remove 'TITLE=' and '\n'
    ...
    def _parse_spectrum_ion(self, input_spectrum_handle, input_line):
        mz_list = []
        intensity_list = []
        # line = input_spectrum_handle.readline()
        line = input_line  # 先读取本行，而后再用readline()
    ...
    def getmzandintensitylist(self, input_spectrum_handle):
        ...
        mz_list, intensity_list = self._parse_spectrum_ion(input_spectrum_handle, line)
        return mz_list, intensity_list
