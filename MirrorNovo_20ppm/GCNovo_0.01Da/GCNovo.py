...
if __name__ == '__main__':
    logger.info("Welcome to GCNovo, Please wait a moment...")

    param_name = "config/params"
    # search denovo
    param_path = sys.argv[:]
    # print(param_path)
    ...
    #     p = multiprocessing.Process(target=GCNovo, args=(args,f'{args.denovo_input_feature_file}_tmp{i}',f'{args.denovo_output_file}_file{i}',i))
    #     process.append(p)
    #     p.start()
    # for p in process:
    #     p.join()
    ...
    change_format(args.denovo_output_file + ".beamsearch", args.denovo_output_file + ".beamsearch.txt", n=args.beam_size)
    logger.info(f'using time:{time.time() - time1}')
