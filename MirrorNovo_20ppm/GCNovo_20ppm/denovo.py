...
from model_gcn import Direction, InferenceModelWrapper
from cython_modules import get_ion_index
from data_reader import GCNovoDenovoDataset, chunks
...
class IonCNNDenovo(object):
    ...
    def _beam_search(self, model_wrapper: InferenceModelWrapper,
        ...
        # batch_precursormass = torch.from_numpy(batch_precursormass).to(self.device)
        # batch_spectrum_representation = torch.from_numpy(batch_spectrum_representation).to(self.device)

        batch_peak_location = torch.tensor(
            [x.peak_location for x in feature_dp_batch],
            dtype=torch.float32,
            device=self.device
        )

        batch_peak_intensity = torch.tensor(
            [x.peak_intensity for x in feature_dp_batch],
            dtype=torch.float32,
            device=self.device
        )

        batch_precursormass = torch.tensor(
            [x.precursormass for x in feature_dp_batch],
            dtype=torch.float32,
            device=self.device
        )

        batch_spectrum_representation = torch.tensor(
            [x.spectrum_representation for x in feature_dp_batch],
            dtype=torch.float32,
            device=self.device
        )
        # batch_peak_location = torch.tensor([x.peak_location for x in feature_dp_batch], dtype=torch.float32, pin_memory=True)
        # batch_peak_location = batch_peak_location.to(self.device, non_blocking=True)
        #
        #
        # batch_peak_intensity = torch.tensor([x.peak_intensity for x in feature_dp_batch], dtype=torch.float32, pin_memory=True)
        # batch_peak_intensity = batch_peak_intensity.to(self.device, non_blocking=True)
        #
        #
        # batch_precursormass = torch.tensor([x.precursormass for x in feature_dp_batch], dtype=torch.float32, pin_memory=True)
        # batch_precursormass = batch_precursormass.to(self.device, non_blocking=True)
        #
        # batch_spectrum_representation = torch.tensor([x.spectrum_representation for x in feature_dp_batch], dtype=torch.float32, pin_memory=True)
        # batch_spectrum_representation = batch_spectrum_representation.to(self.device, non_blocking=True)
        ...
        # block_knapsack_candidates = [[i for i in range(3, 24)],[i for i in range(3, 24)]]
        while True:
            ...
            # block_ion_location = torch.tensor(block_ion_location, pin_memory=True)
            # block_ion_location = block_ion_location.to(self.device, non_blocking=True)
    ...
    def _select_path(feature_dp_batch: list, top_candidate_batch: list) -> tuple:
        """
        for each feature, select the best denovo sequence given by GCNovo model
        :param feature_dp_batch: list of DenovoData
        ...
        for feature_index in range(feature_batch_size):
            ...
            for beam_search_sequence in candidate_list:
                ...
                sequence_mass += config.mass_ID[config.GO_ID] + config.mass_ID[
                    config.EOS_ID]
                if abs(sequence_mass - precursor_mass)*1e6/sequence_mass <= config.PRECURSOR_MASS_PRECISION_TOLERANCE:
    ...
    def search_denovo(self, model_wrapper: InferenceModelWrapper,
                      beam_search_reader: GCNovoDenovoDataset, denovo_writer: DenovoWriter,device):
        ...
        for index, feature_batch_index in enumerate(test_set_iter):
            ...
            logger.info("{},Read {}th/{} batches".format(device,index, total_batch_num))
