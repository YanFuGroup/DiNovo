...
class WorkerTest(object):
    ...
    def __init__(self, args):
        ...
        # we currently use GCnovo_config to store both const & settings
        ...
        self.predicted_format = GCnovo_config.predicted_format
    ...
    def test_accuracy(self, db_peptide_list=None):
        ...
        # read predicted peptides from GCnovo or peaks
        if GCnovo_config.predicted_format == "GCnovo":
            self._get_predicted()
        else:
        ...
        for index, predicted in enumerate(self.predicted_list):
            ...
            if feature_id in target_dict_db_mass:
                ...
                for predicted_sequence, predicted_score in zip(predicted["sequence"], predicted["score"]):
                    predicted_AA_id = [GCnovo_config.vocab[x] for x in predicted_sequence]
                    target_AA_id = [GCnovo_config.vocab[x] for x in target]
    ...
    def _compute_peptide_mass(self, peptide):
        ...
        peptide_mass = (GCnovo_config.mass_N_terminus
                        + sum(GCnovo_config.mass_AA[aa] for aa in peptide)
                        + GCnovo_config.mass_C_terminus)
    ...
    def _get_predicted(self):
        ...
        predicted_list = []
        col_feature_id = GCnovo_config.pcol_feature_id
        col_feature_area = GCnovo_config.pcol_feature_area
        col_sequence = GCnovo_config.pcol_sequence
        col_score = GCnovo_config.pcol_score
        col_scan_list_middle = GCnovo_config.pcol_scan_list_middle
        col_scan_list_original = GCnovo_config.pcol_scan_list_original
    ...
    def _get_target(self):
        ...
        with open(self.target_file, 'r') as handle:
            ...
            raw_sequence_index = header.index(GCnovo_config.col_raw_sequence)
            for line in handle:
                line = re.split(',|\r|\n', line)
                feature_id = line[0]
    ...
    def _match_AA_novor(self, target, predicted):
        ...
        target_mass = [GCnovo_config.mass_ID[x] for x in target]
        target_mass_cum = np.cumsum(target_mass)
        predicted_mass = [GCnovo_config.mass_ID[x] for x in predicted]
